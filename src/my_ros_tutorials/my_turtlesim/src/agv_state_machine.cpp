...
void Turtle::agv_state_machine()
{
    ...
    db50_agv_.Sys.Agv_Start =   (!db50_agv_.Sys.Agv_Run_Proc) &&
                                (db51_tcp_.Tx_Agv.General.taskctrl > 0) && 
                                (db51_tcp_.Tx_Agv.General.nodenum > 0);

    //这里判断上位机给出的enable,并且马上清零
    if(db51_tcp_.Tx_Agv.General.taskctrl> 0)
    {
        db51_tcp_.Tx_Agv.General.taskctrl= 0; 
        modbus_mapping_->tab_registers[34]=0;
    }
        ...
        db50_agv_.Int_Step == 25)
    {
                   ...
                   (!db50_agv_.Status.Rotate_Fwd && db50_agv_.Status.Rotate_Bwd)) 
        {
            if (!db50_agv_.Excute.Rotation_Start && !client_walk_absolute_goal_handle_) 
            {
                ...
                RCLCPP_INFO(nh_->get_logger(), "[%s] step25: 发布ros旋转action(x=%0.2f,y=%0.2f,a=%0.2f,v=%0.2f,x1y2r3=%d)", real_name.c_str(),goal_msg.x,goal_msg.y,goal_msg.theta,goal_msg.vel, goal_msg.x1_y2_rote3 );
